...
class SparseArguments:
    ...
    bottleneck_dim: Optional[int] = field(
        default=12, metadata={"help": "matrix rank in lora"}
    )
...
def main():
    ...
    from opendelta.delta_models.bitfit import BitFitConfig, BitFitModel
    import json
    bitfit_config = json.load(open("./bitfit_config.json"))
    bitfit_config = BitFitConfig.from_dict(bitfit_config)
    delta_model = BitFitModel.from_config(bitfit_config, backbone_model=model)
